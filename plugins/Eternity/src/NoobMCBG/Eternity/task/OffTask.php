            ...
            foreach($this->getPlugin()->cooldown->getAll() as $player => $cooldown){
                if($cooldown == 0){
                    ...
                    if($this->getPlugin()->getServer()->getPlayer($player) !== null){
                        $this->getPlugin()->getServer()->getPlayer($player)->sendMessage("§l§c•§e Kĩ Năng§a Eternity§e Của Bạn Đã Hết Thời Gian Sử Dụng");
                    }
                    continue;
                }
                if($this->getPlugin()->getServer()->getPlayer($player) !== null){
                    $this->getPlugin()->getServer()->getPlayer($player)->sendMessage("§l§c•§e Kĩ Năng§a Eternity§e Của Bạn Còn§b $cooldown Phút");
                }
