...
class SortTask extends AsyncTask{
	private $sender, $tokenData, $addOp, $page, $ops, $banList;
	 ...
	 * @param array				$banList
	 */
	public function __construct(string $sender, array $tokenData, bool $addOp, int $page, array $ops, array $banList){
		$this->sender = $sender;
	...
	private function getTopList(){
		...
		$n = 1;
		$this->max = ceil((count($toptoken) - count($banList) - ($this->addOp ? 0 : count($ops))) / 5);
		$this->page = (int)min($this->max, max(1, $this->page));

		foreach($toptoken as $p => $m){
			$p = strtolower($p);
	...
	public function onCompletion(Server $server){
		if($this->sender === "CONSOLE" or ($player = $server->getPlayerExact($this->sender)) instanceof Player){ // TODO: Rcon
			$plugin = TokenAPI::getInstance();

			$output = ($plugin->getMessage("toptoken-tag", [$this->page, $this->max], $this->sender)."\n");
			$message = ($plugin->getMessage("toptoken-format", [], $this->sender)."\n");
