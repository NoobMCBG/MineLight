...
class PureChat extends PluginBase
{
    ...
    */
    ...
    public function applyPCTags($string, Player $player, $message, $levelName)
    {
		...
		if (($fcaClan = Server::getInstance()->getPluginManager()->getPlugin("GCPClan")) !== null) {
            if ($fcaClan->haveClan($player)) {
                $profile = $fcaClan->getProfile($player);
                $string = str_replace("{clan_name}", $fcaClan->getClanName($player), $string);
                $string = str_replace("{clan_tag}", $fcaClan->getClanTag($player), $string);
                $string = str_replace("{clan_motd}", $fcaClan->getClan($profile->get("clan"))["motd"], $string);
                $string = str_replace("{clan_rank_name}", $fcaClan->getRankName($profile->get("rank")), $string);
            } else {
                $string = str_replace("{clan_name}", "", $string);
                $string = str_replace("{clan_tag}", "", $string);
                $string = str_replace("{clan_motd}", "", $string);
                $string = str_replace("{clan_rank_name}", "", $string);
            }
        }
